            ...
            if policy.get("serviceName") == policy_tmp.get("serviceName"):
                ...
                for ue in policy.get("allowed_users"):
                    if ue not in policy_tmp.get("allowed_users"):
                        ...
                        addUE(ue_ip, policy.get("ip"), policy.get("port")) #[!] how to handle ue_ip \w auth different from ip?
...
#add a new policy (old service, new allowed user) 
def addUE(ue_ip, service_ip, port):
    te = sh.TableEntry('my_ingress.ipv4_exact')(action='my_ingress.ipv4_forward')
    te.match["hdr.ipv4.srcAddr"] = ue_ip
    te.match["hdr.ipv4.dstAddr"] = service_ip
    te.action["port"] = port
    te.insert()
